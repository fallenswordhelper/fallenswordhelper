...
n.forEach(U(Vt,!0)),await V(W(30,t).map(Ft)),n.forEach(U(Vt,!1))}function Yt(t){const{itemId:n,invId:e}=F(O,t.dataset.tipped)
return[Number(n),Number(e)]}const zt=t=>[ot(t),...Yt(t)]
function Jt(t){Ot(Y("img",t).map(zt))}const Kt=t=>z(st).get(t.detail).get("del"),Zt=(t,n)=>ot(J(`img[data-tipped*="&inv_id=${n}&"]`,t))
function tn(t,n){Ot(Kt(n).map((e=>[Zt(t,e),n.detail,e])))}function nn(t){"IMG"===t.target.tagName&&(t.preventDefault(),h("components","quickDel"),Z(t.target),Ot([[ot(t.target),...Yt(t.target)]]))}function en(t){t.classList.add("fshQuickDel"),K(t,nn)}function cn(){const t=function(){const t=tt("profileRightColumn"),n=nt("inventory-table",t)
...
//# sourceMappingURL=components-CqsmQ-xi.js.map
