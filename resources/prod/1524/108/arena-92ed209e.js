import{a as n,bI as t,j as a,C as e,c as s,bJ as r,r as o,bK as i,bL as c,E as f,w as l,n as u,bM as d,bN as p}from"./calfSystem-34913441.js"
...
import{s as b,g}from"./idb-ec5d6344.js"
...
import{s as S}from"./setTipped-cc91a1bc.js"
function E(t){return n({cmd:"arena",...t})}function F(){return E({subcmd:"view"})}function q(n,t,[a,e]){const s=n.find((([,n])=>n===a))
return s&&(y(s[0]).style.backgroundColor="#ff0000",t[a]=e),t}let C=0,J=0
const N=()=>C,D=()=>J
function K(){b(x,C)}function P(n,t){C=C||{},C.minLvl=n,C.maxLvl=t,K()}function R(){$('#arenaTypeTabs table[width="635"]').DataTable().draw()}function z(){w(P,R)}function A(){P(r.arenaMinLvl,r.arenaMaxLvl),$("#fshMinLvl").val(C.minLvl),$("#fshMaxLvl").val(C.maxLvl),R()}function B(n){C=C||{},C.hideMoves=n.target.checked,K(),$(".moveMax").toggle(!n.target.checked)}function G(n,t){return!N()||function(n){const t=N().minLvl,a=N().maxLvl,e=k(n[7])
return j(_,e,t,a)}(t)}function H(n,t,a,e){return!0}function I(n,t,a,e){$(`#${n}`,t).val(N()?.[a]??r[e])}function O(){const n=function(){const n=$("#pCC > table > tbody > tr:nth-child(4)")
return n.clone().insertBefore(n).find("td").attr("height","2"),n.clone().insertAfter(n).find("td").attr("height","1"),n}(),t=$(L)
!function(n){const t=$("#fshHideMoves",n)
null!=N()?.hideMoves&&(t.prop("checked",N().hideMoves),$(".moveMax").toggle(!N().hideMoves)),t.on("click",B)}(t),function(n){I("fshMinLvl",n,"minLvl","arenaMinLvl")}(t),function(n){I("fshMaxLvl",n,"maxLvl","arenaMaxLvl")}(t),function(n){$("#fshMinLvl, #fshMaxLvl",n).on("keyup",z),$("#fshReset",n).on("click",A)}(t),$("td",n).append(t)}function V(n,a){const e=o(/#\s(?<id>\d+)/,a.eq(0).text())
e&&t(N()?.id)&&(N().id[e]=e,function(n,t){D()?.[t]||(n.css("background-color","#F5F298"),n.find("tr").css("background-color","#F5F298"))}(n,e))}function Q(n,t){const a=o(/(?<move>\d)\.png/,$("img",t).attr("src"))
a&&$(t).attr("data-order",a)}function U(n,t){const a=o(M,$("img",n).attr("src"))
...
N()?.moves&&U(a,t)}(e,a),function(n){const t=n.eq(8)
...
var a}
function an(n,[t,,a]){a&&tn(n,a)&&nn(t)}function en(n,t,a){S(a.players.map(s(Z,n)).join("<br>"),t),t.classList.add("tip-static"),n&&"Join"===t.value&&function(n,t,a){a.players.filter((t=>t.guild_id===n)).length===a.max_players/4&&nn(t)}(n,t,a)}const sn=[t,n=>f(n.players),n=>n.players.length>0]
...
$("a",a).contents().unwrap(),$(t).prepend($("<thead/>").append(a))}function un(){O(),K(),$.fn.dataTable.ext.search.push(G),$.fn.dataTable.ext.search.push(H)}function dn(n,[r,o,i]){const c=$('table[width="635"]',n)
...
1===n.length?async function(n){pn(n,await m([g(x),g("fsh_arenaFull"),F().catch((()=>({}))),v()]))}(n):u("arena","Join error screen ?")}var hn=Object.freeze({__proto__:null,default:mn})
export{mn as a,E as b,hn as c,F as v}
//# sourceMappingURL=arena-92ed209e.js.map
