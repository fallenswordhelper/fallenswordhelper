...
l(n+1,t),e===n+1&&Y("Done.","blue")}()}let st=0,at=0,ot=0,rt=0,it=0,ct=0,lt=0
function ft(t,e){l(t,c(e))}async function ut(t){!function(t,e){nt(e,{href:t,findBuffNicks:st})}(t,await T(t))}function dt(){ft(ot.length,"potentialBuffers"),ot.length<=0?Y("Done.","blue"):(Y("Parsing player data ...","green"),ot.forEach(ut))}function pt(t,e){E()!==k(e)&&ot.push(t)}function bt(t,e){if(n=function(t){return parseInt($(t).find("td:eq(2)").text().replace(/,/g,""),10)}(e),n>=at&&n>=Q()){pt($(e).find("td:eq(1) a").attr("href"),$(e).find("td:eq(1) a").text())}var n}async function ht(t,n,s){const a=function(t,e){return 1===t?Math.round(rt*e/50):t+1}(t,n)
...
n<s?ht(n,s,mt):dt()}async function gt(){if(rt=parseInt(c("onlinePlayers").value,10),0!==rt){mt(await e(1))}else dt()}function yt(t){const{tipped:e}=t.dataset,{mins:n}=N(e);(function(t,e,n){return t<5&&e>=at&&e>=n})(n,Number(b(I,e)),Q())&&pt(t.href,u(t))}function $t(t){ct.push(S+t)}async function vt(t){!function(t){const e=f(t)
j('#profileLeftColumn a[data-tipped*="Last Activity"]',e).forEach(yt),lt+=1,lt===ct.length&&gt()}(await T(t))}function xt(t){const e=f(t)
c("guildMembers").checked&&j('#pCC a[data-tipped*="<td>VL:</td>"]',e).forEach(yt),ct=[],ct.push(B),A(it).forEach($t),lt=0,c("alliesEnemies").checked?ct.forEach(vt):gt()}function wt(t,e){return 0!==e}function kt(t){t.deleteRow(-1)}function Lt(){!function(){const t=c("buffTable")
P(t.rows).filter(wt).forEach(h(kt,t))}(),ft("","buffNicks"),Y("Idle.","black"),ft("","potentialBuffers"),ft("0","buffersProcessed")}function Pt(e){C()||(ft(st,"buffNicks"),Y(`Gathering list of ${e} ...`,"green"),_=c("level175").checked,ft("0","buffersProcessed"),ot=[],it=c("extraProfile").value,L("extraProfile",it),async function(){xt(await t())}())}function Ct(t,e){return t===e.id}function jt(){const t=parseInt($("#selectedBuff").val(),10),e=R.find(h(Ct,t))
st=e.nicks,at=e.lvl,Pt("potential buffers")}function It(){const t=$("#textToSearchFor").val().split(",").map(k).join(",")
L("textToSearchFor",t),st=t,at=1,Pt("profiles to search")}function Bt(){it=i("extraProfile")}function Et(t){w(c("findbuffsbutton"),t,!0)}function St(){w(c("clearresultsbutton"),Lt,!0)}function Tt(t){const e=t||x()
o.sortBy="name",o.sortAsc=!0,R.sort(q),Bt(),l(Z(O,it),e),X(),Et(jt),St()}function At(t){const e=t||x()
Bt(),l(Z(V,it),e),X(),Et(It),St()}export{Tt as injectFindBuffs,At as injectFindOther}
//# sourceMappingURL=findBuffs-74fb8fc3.js.map
