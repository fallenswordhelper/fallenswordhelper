...
function evalHolyFlame(combat) {
  ...
  combat.holyFlameBonusDamage = Math.max(
    Math.floor(
      (combat.player.damageValue - combat.creature.armor) *
        combat.player.holyFlameLevel *
        0.002,
    ),
    0,
  );
  if (combat.player.holyFlameLevel > 0) {
    combat.extraNotes += `HF Bonus Damage = ${combat.holyFlameBonusDamage}<br>`;
  }
}

function calcDdBonus(combat) {
  combat.deathDealerBonusDamage = Math.floor(
    combat.player.damageValue *
      (Math.min(
        Math.floor(combat.player.killStreakValue / 5) *
          0.01 *
          combat.player.deathDealerLevel,
        20,
      ) /
        100),
  );
...
function calcCaBonus(combat) {
  combat.counterAttackBonusAttack = Math.floor(
    combat.player.attackValue * 0.0025 * combat.player.counterAttackLevel,
  );
  combat.counterAttackBonusDamage = Math.floor(
    combat.player.damageValue * 0.0025 * combat.player.counterAttackLevel,
  );
}
