...
export const calcDefWithConst = (def, cnsttn) =>
  Math.ceil(def * (1 + fallback(cnsttn, 0) * 0.001));

const calcDcEffect = (points) => 1 - points * darkCurseMultiplier;
export const calcDcValue = (buffedDefense, lvl) =>
  Math.ceil(buffedDefense * calcDcEffect(lvl));
