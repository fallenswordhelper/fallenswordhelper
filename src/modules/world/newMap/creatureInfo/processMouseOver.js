...
function genericColumns([title, value, yours, yourClass, getStatFn]) {
  return [
    titleCell(title),
    valueCell(
      `${value} (your ${yours}:<span class="${yourClass}">${getStatFn()}</span>)`,
    ),
  ];
}

function genericRow(incomingArray) {
  return ['<tr>', ...incomingArray, '</tr>'].join('');
}
...
function genericStatRow(first, second) {
  return genericRow([...genericColumns(first), ...genericColumns(second)]);
}
...
function tipHp(creature, oneHitNumber) {
  return genericRow([
    titleCell('HP'),
    valueCell(
      `${creature.hp} (your HP:<span class="fshYellow">${getStatHp()}</span>)(1H: <span class="fshRed">${oneHitNumber}</span>)`,
    ),
    titleCell('Gold'),
...
function doMouseOver(creature) {
  const oneHitNumber = Math.ceil(
    creature.hp * calf.hpVariable + creature.armor * calf.generalVariable,
  );
  let myLvlClas = 'fshYellow';
  getMyStats();
  if (getStatLevel() > creature.level) {
    myLvlClas = 'fshRed';
  }
