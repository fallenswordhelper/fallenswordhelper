...
export function helpLink(title, text) {
  return (
    '&nbsp;[&nbsp;<span class="fshLink tip-static" data-tipped="' +
    `<span class='fshHelpTitle'>${title}</span><br><br>${text}">?</span>&nbsp;]`
  );
}
...
const stability = (unstable) =>
  unstable
    ? '<span class="fsh-setting-unstable" data-tooltip="Warning: Causes page instability"></span>'
    : '';

export function justLabel(name) {
  const { helpText, helpTitle, network, title, unstable } =
    mySimpleCheckboxes[name];
  return `${hasNetwork(network)}${stability(unstable)}<label class="fshNoWrap" for="${name}">${fallback(
    title,
    helpTitle,
  )}${helpLink(helpTitle, helpText)}:</label>`;
}
