...
function sum(statObj, acc, curr) {
  return acc + getVal(curr, statObj);
}

function calcTotalStats(statObj) {
  return ['Attack', 'Defense', 'Armor', 'Damage', 'HP'].reduce(
    partial(sum, statObj),
    0,
  );
}
...
function fshDataFilter(data) {
  ...
  getArrayByTagName('font', container)
    .filter(contains('Bonuses'))
    .forEach(addStats);
...
export default function addStatTotalToMouseover() {
  // jQuery
