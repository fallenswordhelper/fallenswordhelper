...
function buildItemHash(invItems) {
  return {
    ...invItems.map(([, i]) => i.item_id).reduce(tally, {}),
...
const tooltip = (type) =>
  ` data-tooltip="INSTANTLY ${
    type
  } THE ITEM. NO REFUNDS OR DO-OVERS! Use at own risk."`;

const generalButton = (className, extra, text) =>
  ` [<button class="fshStoreItemsButton ${
    className
  }"${extra}>${text}</button>]`;

const actionButton = (color, type, label) =>
  generalButton(
...
function doAhLinks(invItem) {
  ...
  return aLink(
    `${ahSearchUrl}${encodeURIComponent(invItem.item_name)}`,
    '',
    'AH',
  );
}

const doUfsgLink = (invItem) =>
  aLink(
...
const hasMultiple = (prefs, itemHash, invItem) =>
  prefs[checkAllOfType] && itemHash[invItem.item_id] > 1;

const canSend = (prefs, invItem) =>
  prefs[showQuickSendLinks] && (!invItem.bound || invItem.guild_tag !== -1);

const canDrop = (prefs, invItem) =>
  prefs[showQuickDropLinks] && invItem.guild_tag === -1;
...
function getConditionalArray(prefs, itemHash, invItem) {
  return [
    [
      () => prefs[showExtraLinks],
      () => `${doAhLinks(invItem)} ${doUfsgLink(invItem)}`,
    ],
    ...
    [
      () => canSend(prefs, invItem),
      () => actionButton('fshBlue', 'SENDS', 'Send'),
    ],
    [
      () => canDrop(prefs, invItem),
      () => actionButton('fshRed', 'DROP', 'Drop'),
    ],
  ];
}
...
function decorateItems(prefs, itemHash, [inject, invItem]) {
  ...
  const newInner = getConditionalArray(prefs, itemHash, invItem).reduce(
    buildNewInner,
    '',
  );
