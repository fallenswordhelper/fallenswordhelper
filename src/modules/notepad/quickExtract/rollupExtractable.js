...
const tip = (playerId, itemId, invId) =>
  `fetchitem.php?item_id=${itemId}&inv_id=${invId}&t=1&p=${playerId}`;
...
export default function rollupExtractable(playerId, filteredExtractable) {
  return uniq(filteredExtractable, 'item_name').map((i) => ({
    ...i,
    count: filteredExtractable.filter((e) => e.item_name === i.item_name)
      .length,
    delPending: false,
    extractIds: filteredExtractable
      .filter((e) => e.item_name === i.item_name)
      .map((e) => e.inv_id),
