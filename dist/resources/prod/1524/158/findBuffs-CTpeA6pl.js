...
i(n+1,t),e===n+1&&Y("Done.","blue")}()}let at=0,rt=0,ot=0,it=0,ct=0,lt=0,ft=0
function ut(t,e){i(t,o(e))}async function dt(t){!function(t,e){st(e,{href:t,findBuffNicks:at})}(t,await D(t))}function pt(){ut(ot.length,"potentialBuffers"),ot.length<=0?Y("Done.","blue"):(Y("Parsing player data ...","green"),ot.forEach(dt))}function bt(t,e){A()!==L(e)&&ot.push(t)}function ht(t,e){if(n=function(t){return parseInt($(t).find("td:eq(2)").text().replace(/,/g,""),10)}(e),n>=rt&&n>=J()){bt($(e).find("td:eq(1) a").attr("href"),$(e).find("td:eq(1) a").text())}var n}async function mt(t,e,n){const s=function(t,e){return 1===t?Math.round(it*e/50):t+1}(t,e)
...
n<s?mt(n,s,yt):pt()}async function gt(){if(it=parseInt(o("onlinePlayers").value,10),0!==it){yt(await R(1))}else pt()}function $t(t){const{tipped:e}=t.dataset,{mins:n}=I(e);(function(t,e,n){return t<5&&e>=rt&&e>=n})(n,Number(h(S,e)),J())&&bt(t.href,l(t))}function vt(t){lt.push(j+t)}async function xt(t){!function(t){const e=c(t)
E('#profileLeftColumn a[data-tipped*="Last Activity"]',e).forEach($t),ft+=1,ft===lt.length&&gt()}(await D(t))}function wt(t){const e=c(t)
o("guildMembers").checked&&E('#pCC a[data-tipped*="<td>VL:</td>"]',e).forEach($t),lt=[],lt.push(T),N(ct).forEach(vt),ft=0,o("alliesEnemies").checked?lt.forEach(xt):gt()}function kt(t,e){return 0!==e}function Lt(t){t.deleteRow(-1)}function Pt(){w("find buffs","clear results"),function(){const t=o("buffTable")
k(t.rows).filter(kt).forEach(m(Lt,t))}(),ut("","buffNicks"),Y("Idle.","black"),ut("","potentialBuffers"),ut("0","buffersProcessed")}function Ct(t){C()||(ut(at,"buffNicks"),Y(`Gathering list of ${t} ...`,"green"),Z=o("level175").checked,ut("0","buffersProcessed"),ot=[],ct=o("extraProfile").value,P("extraProfile",ct),async function(){wt(await B())}())}function Bt(t,e){return t===e.id}function Et(){w("find buffs","findBuffsStart")
const t=parseInt($("#selectedBuff").val(),10),e=F.find(m(Bt,t))
at=e.nicks,rt=e.lvl,Ct("potential buffers")}function It(){w("find buffs","findOtherStart")
const t=$("#textToSearchFor").val().split(",").map(L).join(",")
P("textToSearchFor",t),at=t,rt=1,Ct("profiles to search")}function St(){ct=r("extraProfile")}function Tt(t){x(o("findbuffsbutton"),t,!0)}function Nt(){x(o("clearresultsbutton"),Pt,!0)}function At(t){const e=t||v()
s.sortBy="name",s.sortAsc=!0,F.sort(V),St(),i(K(G,ct),e),U(),Tt(Et),Nt()}function jt(t){const e=t||v()
St(),i(K(_,ct),e),U(),Tt(It),Nt()}export{At as injectFindBuffs,jt as injectFindOther}
//# sourceMappingURL=findBuffs-CTpeA6pl.js.map
